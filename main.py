...
def new():
    ...
    try:
        ...
        Label(window, image = login_img).place(x = 5, y =25)
    ...
    name = Label(window, text="Welcome to 99 Customs", font=custom_font, fg="light blue", background="white", image= login_img, compound="bottom")
    name.place(x=5, y=25)
    ...
    submit = Button(window, text="Submit", command=click)  # You can define the `click` function if needed
    submit.place(x=650, y=430)

def click():
    # You can add functionality for the submit button here
